...
def get_config():
    ...
    C.model.positional_encoding = 'APE'
    C.model.layer_norm_placement = 'Pre-LN'
...
if __name__ == '__main__':
    ...
    def batch_end_callback(trainer):
        ...
        # trajectory.append(round(trainer.loss.item(),5))
        if (trainer.iter_num+1) % 10 == 0:
            print(f"iter_dt {trainer.iter_dt * 1000:.2f}ms; iter {trainer.iter_num+1}: train loss {trainer.loss.item():.5f}")


        if (trainer.iter_num+1) % 500 == 0:
    ...
    # print('Type of Encoding: APE_No')
    # trajectory = []

    # losses = trainer.run() ## Code in helper file

    # with open('APE_No_trajectory_loss.pkl', 'wb') as f:
    #     pickle.dump(trajectory, f)

    model.load_state_dict(torch.load('out/adder/final_model.pt'))
    model.eval()
    with torch.no_grad():
        model(torch.tensor([[2,0,2,4,4]], dtype=torch.long))
